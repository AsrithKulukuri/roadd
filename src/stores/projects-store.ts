  ...
  persist(
    (set, get) => ({
        ...
        inFlightProjectsPromise = (async () => {
          ...
          try {
            ...
            const queryPromise = (async () => {
              let response = await fetch("/api/listings?type=project" + scope, { signal: currentSignal, cache: "no-store" });
              if (response.status === 403 && scope) {
                // If admin session is not active or expired, fallback gracefully to public listings
                response = await fetch("/api/listings?type=project", { signal: currentSignal, cache: "no-store" });
              }
      ...
      updateRefId: async (id: string, refId: string) => {
        ...
        try {
          await saveProjectMutation(id, { location: updatedLocation });
          return true;
        } catch (error: unknown) {
          set((state) => ({
            projects: state.projects.map((item) => item.id === id ? project : item),
