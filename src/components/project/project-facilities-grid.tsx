    ...
    <div className="bg-white dark:bg-bg-card border border-border-default rounded-3xl p-5 sm:p-6 shadow-sm">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2.5 sm:gap-3.5">
        {previewList.map((facility, idx) => {
          const { icon: Icon, colorClass, bgClass } = getFacilityMeta(facility);
          return (
            <div
              key={`${facility}-${idx}`}
              className={`p-3 sm:p-4 rounded-2xl border ${bgClass} flex flex-col items-center justify-center text-center gap-2.5 transition-all duration-300 hover:scale-105 hover:shadow-md cursor-default group`}
            >
              {/* Icon Container */}
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center bg-white/80 dark:bg-slate-900/80 shadow-xs group-hover:scale-110 transition-transform">
                <Icon className={`w-5 h-5 sm:w-6 sm:h-6 ${colorClass}`} />
              </div>

              {/* Title */}
              <span className="text-[11px] sm:text-xs font-bold text-text-primary line-clamp-2 leading-snug">
                {facility}
              </span>
            </div>
          );
            ...
            {facilities.map((facility, idx) => {
              ...
              return (
                <div
                  ...
                  className={`p-3 rounded-2xl border ${bgClass} flex flex-col items-center justify-center text-center gap-2 hover:shadow-md transition-all`}
                >
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center bg-white/90 dark:bg-slate-900/90 shadow-xs">
