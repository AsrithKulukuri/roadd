...
import {
  MapPin, CheckCircle2, Download, FileText, ExternalLink,
...
export function ProjectDetailView({ 
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-bg-primary pt-16 pb-24 md:pb-0">
              ...
              <div className="bg-white dark:bg-bg-card border border-border-default rounded-3xl p-5 sm:p-6 shadow-sm space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {project.reraApproved && (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-black bg-slate-950 text-white dark:bg-white dark:text-slate-950 shadow-xs">
                            <CheckCircle2 className="w-3 h-3 text-emerald-400 dark:text-emerald-600" /> RERA Approved
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap sm:justify-end">
                    ...
                    {project.noBrokerage && (
                      <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-slate-950 text-white border border-slate-800 shadow-xs">
                        <ShieldCheck className="w-3.5 h-3.5 text-[#f1a010] shrink-0" />
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50">
        ...
        >
          ...
          <div
            className="mx-0 bg-white/85 dark:bg-slate-950/90 backdrop-blur-xl border-t border-slate-200/60 dark:border-white/10 shadow-[0_-8px_32px_rgba(0,0,0,0.12)]"
            ...
            <div className="flex items-center gap-3 px-4 pt-3 pb-3">
              ...
              <button
                ...
                onClick={async () => {
                  if (!whatsapp) return;
                  try {
                    await performListingAction("project", project.id, "whatsapp_click");
                    // Open WhatsApp via anchor click — triggers app on mobile without navigating away
                    const a = document.createElement("a");
                    a.href = whatsapp;
                    a.target = "_blank";
                ...
                className={`flex-1 flex items-center justify-center gap-2 h-12 rounded-2xl font-bold text-sm text-white shadow-lg shadow-green-500/25 active:scale-95 transition-all duration-150 ${!whatsapp ? "opacity-50 cursor-not-allowed" : ""}`}
