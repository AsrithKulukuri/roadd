...
export function ShelfCard({
  ...
  if (cardStyle === "compact-marketplace") {
    return (
      ...
      >
        {/* Left Photo */}
        <div className="relative w-full sm:w-[42%] aspect-[4/3] sm:aspect-auto shrink-0 overflow-hidden bg-slate-950">
          <Image
          ...
          <button
            ...
            className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-md flex items-center justify-center text-slate-700 dark:text-slate-200 hover:text-red-500 hover:bg-white transition-all shadow-sm"
          >
          ...
          <div className="pt-2 border-t border-slate-100 dark:border-slate-800/80 space-y-1">
            ...
            <div className="flex items-center justify-between gap-2 text-[11px] text-slate-600 dark:text-slate-400 font-semibold">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                {data.isReadyToMove && (
                  <span className="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 dark:bg-emerald-950/80 dark:text-emerald-300 text-[10px] font-extrabold">
                    Ready
                  </span>
                )}
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // 2. TALL LUXURY PORTRAIT (Style 02: 280–340px wide, 480–560px high)
  // ──────────────────────────────────────────────────────────────────────────
  if (cardStyle === "tall-portrait") {
    return (
      <Link
        href={data.href}
        className={cn(
          "group relative flex flex-col h-full rounded-3xl overflow-hidden bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 shadow-md hover:shadow-2xl hover:border-amber-400/60 transition-all duration-500",
          className
        )}
      >
        {/* Dominant Vertical Architectural Photo */}
        <div className="relative aspect-[3/4] w-full overflow-hidden bg-slate-950">
          ...
          <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
            ...
            <button
              ...
              className="w-8 h-8 rounded-full bg-slate-950/60 backdrop-blur-md border border-white/20 text-white hover:text-red-400 hover:bg-slate-900 transition-all flex items-center justify-center shadow-md"
            >
              <Heart className={cn("w-4 h-4", isSaved && "fill-red-500 text-red-500")} />
            </button>
          </div>

          {/* Photo Bottom Info Overlay */}
          <div className="absolute bottom-3 left-3.5 right-3.5 text-white space-y-1">
            <div className="text-2xl font-black tracking-tight text-white drop-shadow-md">
              ₹{data.priceStr}
            </div>
            <h3 className="font-heading font-extrabold text-lg line-clamp-1 text-white group-hover:text-amber-400 transition-colors">
              {data.title}
            </h3>
            <div className="flex items-center justify-between text-xs text-slate-300 font-medium">
              ...
              {data.imageCount > 1 && (
                <span className="shrink-0 flex items-center gap-1 px-1.5 py-0.5 rounded bg-black/40 backdrop-blur-md text-[10px]">
                  <Camera className="w-2.5 h-2.5" /> {data.imageCount}
                </span>
              )}
            </div>
          </div>
  ...
  if (cardStyle === "luxury-banner") {
    return (
      <Link
        href={data.href}
        className={cn(
          "group relative flex flex-col md:flex-row h-full rounded-3xl overflow-hidden bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 shadow-xl hover:shadow-2xl hover:border-amber-400/60 transition-all duration-500",
          className
        )}
      >
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="w-7 h-7 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:text-red-500 transition-all flex items-center justify-center"
              >
                <Heart className={cn("w-3.5 h-3.5", isSaved && "fill-red-500 text-red-500")} />
              </button>
            </div>
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // 5. BOTTOM FLOATING PANEL (Style 05 / Image 3: 480–640px wide, 420–520px high)
  // ──────────────────────────────────────────────────────────────────────────
  if (cardStyle === "bottom-floating") {
    return (
      <Link
        href={data.href}
        className={cn(
          "group relative flex flex-col h-full rounded-3xl overflow-hidden bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 shadow-xl hover:shadow-2xl hover:border-amber-400/60 transition-all duration-500",
          className
        )}
      >
        {/* Full-bleed Photo */}
        <div className="relative aspect-[4/3] w-full overflow-hidden bg-slate-950">
          ...
          <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
            ...
            <button
              ...
              onClick={handleFavoriteClick}
              aria-label="Save project"
              className="w-8 h-8 rounded-full bg-slate-950/50 backdrop-blur-md text-white hover:text-red-500 hover:bg-white transition-all flex items-center justify-center"
            >
              <Heart className={cn("w-4 h-4", isSaved && "fill-red-500 text-red-500")} />
            </button>
          </div>
      ...
      >
        ...
        <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-950">
          ...
          <button
            ...
            className="absolute top-3 right-3 w-8 h-8 rounded-full bg-slate-950/60 backdrop-blur-md text-white hover:text-red-500 transition-all flex items-center justify-center"
          >
            <Heart className={cn("w-4 h-4", isSaved && "fill-red-500 text-red-500")} />
          </button>
        </div>
        ...
        <div className="p-4 sm:p-5 flex flex-col justify-between flex-1 space-y-3">
          ...
          {/* Amenities Grid */}
          <div className="grid grid-cols-3 gap-1.5 py-2 border-y border-slate-100 dark:border-slate-800 text-[10px] font-bold text-slate-600 dark:text-slate-300">
            <div className="flex items-center gap-1 justify-center p-1.5 rounded-lg bg-slate-50 dark:bg-slate-800/60">
              <Trees className="w-3 h-3 text-emerald-500 shrink-0" /> Private Garden
            </div>
            <div className="flex items-center gap-1 justify-center p-1.5 rounded-lg bg-slate-50 dark:bg-slate-800/60">
              <Building2 className="w-3 h-3 text-amber-500 shrink-0" /> Clubhouse
            </div>
            <div className="flex items-center gap-1 justify-center p-1.5 rounded-lg bg-slate-50 dark:bg-slate-800/60">
              <Shield className="w-3 h-3 text-blue-500 shrink-0" /> 24/7 Security
            </div>
          </div>
  ...
  if (cardStyle === "construction-progress") {
    return (
      <Link
        href={data.href}
        className={cn(
          "group relative flex flex-col h-full rounded-3xl overflow-hidden bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 shadow-md hover:shadow-xl hover:border-amber-400/60 transition-all duration-500",
          className
        )}
      >
        {/* Elevation Photo */}
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // 8. DARK LUXURY EDITORIAL (Style 08: 520–680px wide, 300–400px high)
  // ──────────────────────────────────────────────────────────────────────────
  return (
    <Link
      href={data.href}
      className={cn(
        "group relative flex flex-col md:flex-row h-full rounded-3xl overflow-hidden bg-slate-950 text-white border border-slate-800 shadow-2xl hover:border-amber-500/60 transition-all duration-500",
        className
      )}
    >
      ...
      <div className="relative md:w-[55%] aspect-[16/10] md:aspect-auto min-h-[200px] overflow-hidden bg-slate-900">
        <Image
          src={data.primaryImage}
          alt={data.title}
          fill
