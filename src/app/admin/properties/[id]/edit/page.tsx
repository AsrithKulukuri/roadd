              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    toast.loading("Resolving Google Maps location...", { id: "fetch-maps-edit" });
                    try {
                      const res = await fetch(`/api/resolve-maps-url?url=${encodeURIComponent(trimmed)}`);
                      const data = await res.json();
                      if (data.success && data.latitude && data.longitude) {
                        handlePositionChange(data.latitude, data.longitude);
                        toast.success(`📍 Automatically fetched location coordinates (${data.latitude.toFixed(4)}, ${data.longitude.toFixed(4)})!`, { id: "fetch-maps-edit" });
                      } else {
